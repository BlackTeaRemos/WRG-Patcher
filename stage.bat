...
pushd "%SRC%"
echo === version.dll ===
cl /nologo /O2 /LD /EHsc /std:c++latest /D_CRT_SECURE_NO_WARNINGS /DWRG_RELEASE /I include src\patcher.cpp src\hook.cpp src\redirect.cpp src\overlay.cpp src\overlay_load.cpp src\manifest.cpp src\edat.cpp src\version_gate.cpp src\version_anchor.cpp src\plugin.cpp src\ipc.cpp src\mapdir.cpp src\version_shim.cpp /Fe:version.dll /link /DEF:src\version.def /OUT:version.dll
